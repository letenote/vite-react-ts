...
import { PaymentFormsFilterSchema } from './schema/PaymentFormsFilterSchema.schema';
...
const PaymentFilter = () => {
  ...
  const defaultValues: PaymentFormsFilterType = JSON.parse(
    JSON.stringify(PaymentFormsFilterSchema)
  );
  const form = useForm<PaymentFormsFilterType>({
    defaultValues,
  });
  const { register, formState, handleSubmit, control } = form;
  const { errors } = formState;

  const onValid = (data: PaymentFormsFilterType) => {
    dispatch(
      getPayments({
        page,
        startDate:
        ...
        vendorName: data.name.value.toString(),
      })
    );
  };
...
const MemoizedPaymentFilter = React.memo(
  PaymentFilter,
  (prevProps, nextProps) => {
...
export default MemoizedPaymentFilter;
