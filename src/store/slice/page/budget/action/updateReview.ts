...
type UpdateReviewBudgetParamType = {
  data: {
    budgetId: string;
    isReviewed: boolean;
  };
...
export const updateReviewBudget =
  (param: UpdateReviewBudgetParamType) => async (dispatch: Dispatch) => {
    try {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      await API({
        method: 'post',
        url: `/v1/budgets/update/review`,
        payload: param.data,
      });
      dispatch(
        setSnackbar({
